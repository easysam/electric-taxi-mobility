...
def same_size_matrix(newdf,newdf2,a,b):
    '''
...
def count_transits(df_od,df_od2, a, b, a_t, b_t):
    ...
    duration = datetime.timedelta(minutes=60*24)
    ...
    for interval in range(int(datetime.timedelta(days=1) / duration)):
        ...
        interval_od2=interval_od2.groupby([a, b]).size()
        #interval_od2 = interval_od2[interval_od2>10]

        transit = same_size_matrix(df_od, interval_od2, a, b )
        transit2 = same_size_matrix(interval_od2, df_od, a, b )
...
def p2d_part():
    ...
    df = pd.read_csv("s2_mobility/temp.csv")
    od_with_hs2 = df.set_index(["original_cube","destination_cube"])["pred"]
